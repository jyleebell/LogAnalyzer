...
int main(void) {
	...
	*/
	...
	Arguments:
	  ...
	  - vertex_color1 : An optional color vector for the first graph.  
						If color vectors are given for both graphs, then the subgraph isomorphism is calculated on the colored graphs; 
	  ...
	  - map12 : Pointer to a vector or NULL . If not NULL , then an isomorphic mapping from graph1 to graph2 is stored here.
	  - map21 : Pointer to a vector ot NULL . If not NULL , then an isomorphic mapping from graph2 to graph1 is stored here.
	...
	*/
	...
	Arguments:
	  - graph1 : The first input graph, may be directed or undirected. This is supposed to be the larger graph.
	  - graph2 : The second input graph, it must have the same directedness as graph1 . This issupposed to be the smaller graph.
	  - vertex_color1 : An optional color vector for the first graph.  
						If color vectors are given for both graphs, then the subgraph isomorphism is calculated on the colored graphs; 
						i.e. two vertices can match only if their color also matches. Supply a null pointer here if your graphs are not colored.
	  - vertex_color2 : An optional color vector for the second graph. See the previous argument for explanation.
